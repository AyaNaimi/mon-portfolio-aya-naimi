...
const useImageLoader = (seqRef: MutableRefObject<HTMLUListElement | null>, onLoad: () => void, dependencies: unknown[]) => {
  useEffect(() => {
  ...
  ({
    ...
    const renderLogoItem = useCallback(
      (item: LogoLoopItem, key: string | number) => {
        ...
        const content = isNodeItem ? (
          <span
            className={cx(
              ...
              scaleOnHover && "transition-transform duration-300 ease-[cubic-bezier(0.4,0,0.2,1)] group-hover/item:scale-[1.12]",
            )}
        ...
        const itemAriaLabel = isNodeItem ? item.ariaLabel ?? item.title : item.ariaLabel ?? ("alt" in item ? item.alt : item.title)
